...
import RxCocoa
import RxSwift
class AP_HomeWorkInfoViewModel: BaseViewModel {
    ...
    func getDataList(gradeid:Int,subjectid:Int,datetime:String) -> output{
        ...
        output.requestCommond.subscribe(onNext: { (isReload) in
            ...
            print(self.index)
            ApproveTool.rx.request(ApproveApi.QueryExaminHomeworkList(self.index, 15, gradeid, subjectid, datetime)).asObservable().mapArray(Model_HomeWork.self).subscribe(onNext: { (models) in
