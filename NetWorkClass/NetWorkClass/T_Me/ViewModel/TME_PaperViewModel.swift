...
class TME_PaperViewModel: BaseViewModel {
    ...
    func getDataList(gradeId:Int,subjectid:Int) -> output{
        ...
        output.requestCommond.subscribe(onNext: { (isReload) in
            ...
            TME_Tool.rx.request(TME_API.QueryTeaExamHistory(self.index, 10, gradeId, subjectid)).asObservable().mapArray(Model_TestPaper.self).subscribe(onNext: { (models) in
