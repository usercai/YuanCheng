...
class TME_QuestionViewModel: BaseViewModel {
    ...
    func getDataList() -> output{
        ...
        output.requestCommond.subscribe(onNext: { (isReload) in
            self.index = isReload ? 1 : self.index + 1
                ...
                TME_Tool.rx.request(TME_API.QueryTeaAsk(self.index, 10)).asObservable().mapArray(Model_QuestionList.self).subscribe(onNext: { (models) in
